...
//            
////            if worldManager.isRelocalizationComplete == true {
////                
                        ...
                        DispatchQueue.main.async {
                            for record in records {
                                if let transformData = record["transform"] as? Data {
                                    ...
                                    if !self.parent.sceneView.session.currentFrame!.anchors.contains(where: { simd_equal($0.transform, newAnchor.transform) }) {
