...
struct WelcomeView: View {
    ...
    private func startInitialAnimations() {
        withAnimation(.easeInOut(duration: 1)) {
            circleScale = 0.3      // Circle grows to full size.
            circleOffset = -200       // Circles move to center.
        }
        // 2. Animate the arrow moving into center.
        withAnimation(.easeInOut(duration: 0.75)) {
            arrowOffset = 200        // Arrow moves to center.
            angle = 20
        }
    }
    ...
    private func startAnimations() {
        ...
        withAnimation(.easeInOut(duration: 0.75)) {
           ...
           withAnimation(.interpolatingSpring(stiffness: 500, damping: 3).delay(1.5)) {
           ...
           withAnimation(.spring(duration: 1).delay(2.5)) {
               isShowingIcon = true
               circleScale = 0
           }
           
           // Play the sound effect shortly after the animations start.
    ...
    var body: some View {
        ZStack {
            ...
            if !motionManager.triggerAnimation {
                VStack {
                    AnimateText<ATBlurEffect>($text)
                ...
                .onAppear {
                    ...
                    text = "Move your \(UIDevice.isIpad ? "iPad" : "iPhone") towards right"
...
//                }
        ...
        .onChange(of: motionManager.triggerAnimation) { newValue in
                   if newValue {
                       HapticManager.shared.notification(type: .success)
                       startAnimations()
                   }
               }
        .onChange(of: motionManager.triggerAnimation1) { newValue in
                   if newValue {
                       text = "Almost there!"
